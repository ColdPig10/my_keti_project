...
# self_rag.py
def rag_query(query: str, return_context=False):
    initial_state = {'query': query, 'context': [], 'answer': ''}
    result = graph.invoke(initial_state)
    if return_context:
        return result['answer'], [doc.page_content for doc in result['context']]
    return result['answer']

print("Working Dir:", os.getcwd())
print("Persist DB exists?:", os.path.exists('./pdf_test_chroma_db'))

answer_provenance_prompt = PromptTemplate.from_template("""
당신은 AI 답변 평가자입니다.
아래 '검색 문서 context'와 'AI의 최종 답변'을 보고,
답변이 다음 중 어떤 근거로 만들어졌는지 판정하세요.

- 답변이 오직 context(문서)에서만 온 경우: "문서 기반"
- 답변이 context(문서)와 사전 지식(모델의 사전학습 정보) 둘 다에서 온 경우: "혼합"
- 답변이 context에는 없고, 사전 지식에서만 온 경우: "사전지식 기반"

판정 근거도 간단히 서술하세요.

출력 예시:
{{
  "type": "문서 기반" or "혼합" or "사전지식 기반",
  "reason": "..."
}}
...
provenance_llm = ChatOpenAI(model='gpt-4o-mini', temperature=0)
provenance_chain = answer_provenance_prompt | provenance_llm | StrOutputParser()

def check_answer_provenance(answer: str, context: list):
    context_str = "\n".join(context) if isinstance(context, list) else str(context)
    result = provenance_chain.invoke({"context": context_str, "answer": answer})
    return result